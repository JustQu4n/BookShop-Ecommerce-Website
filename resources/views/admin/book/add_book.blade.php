                    ...
                    <div class="position-center">
                        <form role="form" action="{{ route('book.addPost') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="year" min="1900" max="2022"
                                    id="exampleInputEmail1" value="{{ old('year') }}">
                                @error('year')
                                    <p style="color:red; ">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="total_page" min="1"
                                    id="exampleInputEmail1" value="{{ old('total_page') }}">
                                @error('total_page')
                                    <p style="color:red; ">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                <select class="form-control input-sm m-bot15" name='book_category'>
                                    @foreach ($list_categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('book_category') == $category->id ? 'selected="true"' : false }}>
                                ...
                                <select class="form-control input-sm m-bot15" name='book_brand'>
                                    @foreach ($list_brands as $brand)
                                        <option value="{{ $brand->id }}"
                                            {{ old('book_brand') == $brand->id ? 'selected="true"' : false }}>
                                            {{ $brand->name }}</option>
                                    @endforeach
                                </select>
                                @error('book_brand')
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control input-sm m-bot15" name='book_status'>
                                    ...
                                    <option value="1" {{ old('book_status') == 1 ? 'selected="true"' : false }}>Hiện
                                    </option>
                                </select>
