        ...
        <form class="login" action="{{route('login.')}}" method="POST">
            ...
            <div class="con">
                ...
                <header class="head-form">
                    <a href="#"><img src="{{asset('assets/images/header/New folder/logo.png')}}" style="width: 50%; "alt=""></a>
                    <h2>Đăng nhập</h2>
                    <p>Nhập eamil  và mật khẩu của bạn</p>
                </header>
                <br>
                <div class="field-set">
                    <input class="form-input" id="txt-input" type="text" placeholder="Nhập địa chỉ email" required name="email" value="{{old('email')}}">
                    @error('email')
                        <p style="color:red; margin-left: 18px;   text-transform:none;">{{$message}}</p>
                    @enderror
                    <br>

                    <input class="form-input" type="password" placeholder="Mật khẩu" id="pwd" name="password" required name="password" value="{{old('password')}}">
                    @error('password')
                        <p style="color:red; margin-left: 18px;   text-transform:none;">{{$message}}</p>
                    @enderror
                    <br>
                ...
                <div class="social">
                    <a href="{{route('login.facebook')}}" id="facebook"><img src="{{asset('assets/images/login/fb-btn.svg')}}" alt=""></a>
                    <a href="{{route('login.google')}}"   id="facebook"><img src="{{asset('assets/images/login/gp-btn.svg')}}" alt=""></a>
                </div>
              
                
                <div class="forget-signup">
                    <button><a href="{{route('login.viewPass')}}" style="text-decoration: none;"> Quên mật khẩu</a></button>
                    <div id="register"><a href="{{route('registerindex')}}" style="text-decoration: none;">Đăng ký</a> </div>
                </div>
            </div>
        </form> 
    </div>
